 ...
 * @cdk.githash
 */
public class AndroidTemplateHandler3D {
    ...
    private static AndroidTemplateHandler3D self = null;

    private final ILoggingTool logger = LoggingToolFactory.createLoggingTool(AndroidTemplateHandler3D.class);

    private UniversalIsomorphismTester universalIsomorphismTester = new UniversalIsomorphismTester();

    private AndroidTemplateHandler3D() {
    }

    public static AndroidTemplateHandler3D getInstance() throws CDKException {
        if (self == null) {
            self = new AndroidTemplateHandler3D();
        }
        return self;
    }
