 ...
 * AndroidModelBuilder3D} to place general molecules.
 *
 * @author         chhoppe
 ...
 * @cdk.githash
 * @see AndroidModelBuilder3D
 */
public class AndroidAtomPlacer3D {
    ...
    private final ILoggingTool logger = LoggingToolFactory.createLoggingTool(AndroidAtomPlacer3D.class);
        ...
        for (int index = 0; index < distances.length; index++) {
            if (index == 0) {
                ...
                if (index == 3 && flagBranched) {
                    n2 = AndroidAtomTetrahedralLigandPlacer3D.rotate(n1, bc, DIHEDRAL_BRANCHED_CHAIN);
                } else {
                    n2 = AndroidAtomTetrahedralLigandPlacer3D.rotate(n1, bc, dihedrals[index]);
                }
                n2.normalize();

                Vector3d ba = new Vector3d();
                if (index == 3 && flagBranched) {
                    ba = AndroidAtomTetrahedralLigandPlacer3D.rotate(cd, n2, (-angles[index] / 180) * Math.PI);
                    ba = AndroidAtomTetrahedralLigandPlacer3D.rotate(ba, cd, (-angles[index] / 180) * Math.PI);
                } else {
                    ba = AndroidAtomTetrahedralLigandPlacer3D.rotate(cd, n2, (-angles[index] / 180) * Math.PI);
                }
