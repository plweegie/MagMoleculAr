package com.plweegie.magmolecular.rendering;
 ...
 */
public class AndroidModelBuilder3D {
    ...
    private final AndroidForceFieldConfigurator ffc             = new AndroidForceFieldConfigurator();

    String                                     forceFieldName  = "mm2";

    private static ILoggingTool                logger          = LoggingToolFactory
            .createLoggingTool(AndroidModelBuilder3D.class);
    ...
    public static AndroidModelBuilder3D getInstance(AndroidTemplateHandler3D templateHandler, String ffname,
        ...
        if (!memyselfandi.containsKey(builderCode)) {
            AndroidModelBuilder3D builder = new AndroidModelBuilder3D(templateHandler, ffname, chemObjectBuilder);
    ...
    public IAtomContainer generate3DCoordinates(IAtomContainer molecule, boolean clone) throws CDKException,
        ...
        AndroidAtomPlacer3D ap3d = new AndroidAtomPlacer3D();
        AndroidAtomTetrahedralLigandPlacer3D atlp3d = new AndroidAtomTetrahedralLigandPlacer3D();
     ...
     */
    private void layoutMolecule(List ringSetMolecule, IAtomContainer molecule, AndroidAtomPlacer3D ap3d,
                                AndroidAtomTetrahedralLigandPlacer3D atlp3d, AtomPlacer atomPlacer) throws CDKException, IOException,
    ...
    private void layoutRingSystem(Point3d originalCoord, IAtom placedRingAtom, IRingSet ringSet,
                                  Point3d centerPlacedMolecule, IAtom atomB, AndroidAtomPlacer3D ap3d) {
        ...
        for (int i = 0; i < ac.getAtomCount(); i++) {
            if (!(ac.getAtom(i).getFlag(CDKConstants.ISPLACED))) {
                ...
                ringCenter = AndroidAtomTetrahedralLigandPlacer3D.rotate(ringCenter, n1, angle);
        ...
        for (int i = 1; i < 360; i++) {
            ringCenter = AndroidAtomTetrahedralLigandPlacer3D.rotate(ringCenter, axis, angle);
        ...
        for (int i = 0; i < ac.getAtomCount(); i++) {
            if (!(ac.getAtom(i).getFlag(CDKConstants.ISPLACED))) {
                ...
                ringCenter = AndroidAtomTetrahedralLigandPlacer3D.rotate(ringCenter, axis, rotAngleMax);
    ...
    private void setBranchAtom(IAtomContainer molecule, IAtom unplacedAtom, IAtom atomA, IAtomContainer atomNeighbours,
                               AndroidAtomPlacer3D ap3d, AndroidAtomTetrahedralLigandPlacer3D atlp3d) throws CDKException {
     ...
     */
    private void searchAndPlaceBranches(IAtomContainer molecule, IAtomContainer chain, AndroidAtomPlacer3D ap3d,
                                        AndroidAtomTetrahedralLigandPlacer3D atlp3d, AtomPlacer atomPlacer) throws CDKException {
     ...
     */
    private void placeLinearChains3D(IAtomContainer molecule, IAtomContainer startAtoms, AndroidAtomPlacer3D ap3d,
                                     AndroidAtomTetrahedralLigandPlacer3D atlp3d, AtomPlacer atomPlacer) throws CDKException {
